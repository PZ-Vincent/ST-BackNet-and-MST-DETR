# -*- coding: utf-8 -*-
"""MST-DETR ablation without cyclic time encoding.

The GridSat–ERA5 gated fusion, 3D-CNN temporal-context module and DETR heads are
retained. Month/hour cyclic features are removed only from the lead-time head.
"""
...
class Config:
    # ---------------- Output ----------------
    OUTPUT_DIR = str(OUTPUT_ROOT / "training" / "ablation_no_time_encoding")
    ...
    MODEL_NAME = "MST_DETR_NoTimeCyclicEmbedding"
...
class SpatioTemporalDETR(nn.Module):
    def __init__(self, num_classes: int = 1, num_frames: int = 4):
        ...
        self.time_head = nn.Sequential(nn.Linear(self.hidden_dim, 128), nn.ReLU(), nn.Linear(128, 1))
    ...
    def forward(self, inputs: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
        ...
        # Remove month/hour cyclic features from the lead-time head.
        raw_time = self.time_head(hs_last).squeeze(-1)
...
def run_single_seed(seed: int) -> Dict[str, Any]:
    ...
    print("--- MST_DETR_NoTimeCyclicEmbedding: Remove month/hour cyclic embedding from the lead-time head. | seed={} ---".format(seed))
    ...
    print("Ablation setting          : Remove month/hour cyclic embedding from the lead-time head.")
