# -*- coding: utf-8 -*-
"""MST-DETR ablation without spatially gated multimodal fusion.

GridSat and ERA5 features are aligned and fused by concatenation followed by
convolution. The 3D-CNN temporal-context module, DETR heads and cyclic time
features are unchanged from the full model.
"""
...
class Config:
    # ---------------- Output ----------------
    OUTPUT_DIR = str(OUTPUT_ROOT / "training" / "ablation_no_spatial_gate")
    ...
    MODEL_NAME = "MST_DETR_PlainConcatFusion"
...
class PlainConcatFusion(nn.Module):
    ...
    def forward(self, sat_feat: torch.Tensor, era_feat: torch.Tensor) -> torch.Tensor:
        ...
        era_feat = self.era_proj(era_feat)
        era_feat_up = F.interpolate(era_feat, size=sat_feat.shape[-2:], mode="bilinear", align_corners=False)
        return self.out_conv(torch.cat([sat_feat, era_feat_up], dim=1))
...
class MultiModalBackbone(nn.Module):
    def __init__(self, hidden_dim: int = 256):
        ...
        self.fusion = PlainConcatFusion(sat_dim=hidden_dim, era_dim=128, out_dim=hidden_dim)
...
def run_single_seed(seed: int) -> Dict[str, Any]:
    ...
    print("--- MST_DETR_PlainConcatFusion: Replace GatedAFM with plain concat-conv multimodal fusion. | seed={} ---".format(seed))
    ...
    print("Ablation setting          : Replace GatedAFM with plain concat-conv multimodal fusion.")
