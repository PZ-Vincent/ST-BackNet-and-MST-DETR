# -*- coding: utf-8 -*-
"""GridSat-only MST-DETR ablation.

The ERA5 branch and multimodal fusion are removed. GridSat sequences are
encoded by the satellite backbone and the remaining 3D-CNN temporal-context,
DETR and lead-time components are unchanged.
"""
...
class Config:
    # ---------------- Output ----------------
    OUTPUT_DIR = str(OUTPUT_ROOT / "training" / "ablation_no_era5")
    ...
    MODEL_NAME = "MST_DETR_SatelliteOnly"
...
class SatelliteBackbone(nn.Module):
...
class SpatioTemporalDETR(nn.Module):
    def __init__(self, num_classes: int = 1, num_frames: int = 4):
        ...
        self.backbone = SatelliteBackbone(self.hidden_dim)
...
class MultiModalDataset(torch.utils.data.Dataset):
    ...
    def get_sequence(self, end_time_str: str) -> Optional[List[np.ndarray]]:
        ...
        frames = [[], []]  # sat, time_cyclic
        for i in range(self.seq_len):
            dt = current_dt - timedelta(hours=self.time_interval * i)
            t_str = dt.strftime("%Y%m%d%H")
            sat = self.load_files(t_str)
...
def run_single_seed(seed: int) -> Dict[str, Any]:
    ...
    print("--- MST_DETR_SatelliteOnly: Use GridSat only; ERA5 is not loaded or used by the model. | seed={} ---".format(seed))
    print("Ablation setting          : Use GridSat branch only; ERA5 is not loaded or used by the model.")
