# -*- coding: utf-8 -*-
"""ERA5-only MST-DETR ablation.

The GridSat branch and multimodal fusion are removed. Four-step ERA5 sequences
are encoded by the atmospheric backbone and the remaining 3D-CNN
temporal-context, DETR and lead-time components are unchanged.
"""
...
class Config:
    # ---------------- Output ----------------
    OUTPUT_DIR = str(OUTPUT_ROOT / "training" / "ablation_no_gridsat")
...
class ERA5Backbone(nn.Module):
    def __init__(self, hidden_dim: int = 256):
        ...
        self.era_adapter = nn.Sequential(
            nn.Conv2d(128, hidden_dim, kernel_size=1, bias=False),
            nn.GroupNorm(8, hidden_dim),
            nn.ReLU(inplace=True),
        )

    def forward(self, era_batch: torch.Tensor) -> torch.Tensor:
        era_feat = self.era_backbone(era_batch)
        return self.era_adapter(era_feat)
...
class SpatioTemporalDETR(nn.Module):
    def __init__(self, num_classes: int = 1, num_frames: int = 4):
        ...
        self.backbone = ERA5Backbone(self.hidden_dim)

        # ---------------------------------------------------------------------
        # Lightweight 3D-CNN spatiotemporal context encoder.
        #
        # Input feature sequence after the multimodal backbone:
        #   feat = [B, T, C, Hf, Wf], ordered as [t-9h, t-6h, t-3h, t].
        #
        # Instead of TemporalDiff, all frames including the current time are
        # encoded into a unified spatiotemporal context tensor H_st. H_st is then
        # weakly injected into the current-time feature F_t by a learnable gate:
        #
        #   F_out = F_t + alpha * Gate(F_t, H_st) * H_st
        #
        # This keeps the one-step current feature as the dominant detection
        # representation and avoids the side effects caused by direct diff fusion.
        # ---------------------------------------------------------------------
        self.st_context_encoder = nn.Sequential(
            nn.Conv3d(
                self.hidden_dim,
                self.hidden_dim,
                kernel_size=(3, 3, 3),
        ...
        # Per-time spatial scoring branch. It learns which frame contributes more
        # to the unified context at each spatial location.
        self.st_time_score = nn.Sequential(
            nn.Conv2d(self.hidden_dim, self.hidden_dim // 2, kernel_size=3, padding=1, bias=False),
            nn.GroupNorm(8, self.hidden_dim // 2),
            nn.ReLU(inplace=True),
            nn.Conv2d(self.hidden_dim // 2, 1, kernel_size=1),
        )

        # Channel-spatial gate for injecting H_st into current-time feature F_t.
        self.st_current_gate = nn.Sequential(
        ...
        # Learnable weak residual scale. Initialized small to preserve one-step
        # behavior at the beginning of training.
        self.st_context_scale = nn.Parameter(torch.tensor(0.1))
    ...
    def forward(self, inputs: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
        ...
        B, T, _, H, W = era_keep.shape

        era_stack = era_keep.flatten(0, 1)
        feat = self.backbone(era_stack)
        ...
        # Current-time one-step feature, used as the protected main detection feature.
        # Dataset order is [current, t-3 h, ...]; index 0 is the current frame.
        curr_feat = feat[:, 0]  # [B, C, Hf, Wf]

        # Encode the full spatiotemporal sequence [t-9h, t-6h, t-3h, t].
        # [B, T, C, Hf, Wf] -> [B, C, T, Hf, Wf]
        feat_3d = feat.permute(0, 2, 1, 3, 4).contiguous()
        st_encoded = self.st_context_encoder(feat_3d)  # [B, C, T, Hf, Wf]

        # Learn time weights at each spatial position and aggregate into H_st.
        encoded_list = []
        score_list = []
        for ti in range(T):
            f_i = st_encoded[:, :, ti]          # [B, C, Hf, Wf]
            encoded_list.append(f_i)
            score_i = self.st_time_score(f_i)   # [B, 1, Hf, Wf]
            score_list.append(score_i)

        st_scores = torch.stack(score_list, dim=1)   # [B, T, 1, Hf, Wf]
        st_weights = torch.softmax(st_scores, dim=1)

        st_context = torch.zeros_like(curr_feat)
        for ti, f_i in enumerate(encoded_list):
            st_context = st_context + st_weights[:, ti] * f_i  # [B, C, Hf, Wf]

        # Gate-controlled weak residual fusion:
        #   F_out = F_t + alpha * Gate(F_t, H_st) * H_st
        gate = self.st_current_gate(torch.cat([curr_feat, st_context], dim=1))
        alpha = torch.clamp(self.st_context_scale, 0.0, 1.0)
        src_fused = curr_feat + alpha * gate * st_context

        if not torch.isfinite(src_fused).all():
            raise RuntimeError("src_fused contains NaN/Inf in 3D-CNN ST context fusion.")
...
class MultiModalDataset(torch.utils.data.Dataset):
    def __init__(self, pos_csv_path: str, year_range: List[int], mode: str = "train", only_official: bool = True):
        ...
        for key, group in grouped:
            if not self._sequence_year_valid(key):
                ...
                continue
            required_era5 = [
                os.path.join(self.dir_era_p, "{}.npy".format(key)),
                os.path.join(self.dir_era_s, "{}.npy".format(key)),
                os.path.join(self.dir_era_po, "{}.npy".format(key)),
                os.path.join(self.dir_era_rh_vws, "{}.npy".format(key)),
            ]
            if not all(os.path.exists(path) for path in required_era5):
                skipped_missing_era5_count += 1
                continue
        ...
        print("  -> [positive] skipped by missing ERA5: {}".format(skipped_missing_era5_count))
    ...
    def load_files(self, t_str: str) -> Optional[np.ndarray]:
        ...
        if era_p is None or era_s is None or era_po is None or era_rh_vws is None:
            return None
    ...
    def get_sequence(self, end_time_str: str) -> Optional[List[np.ndarray]]:
        ...
        frames = [[], []]  # era_keep, time_cyclic
        for i in range(self.seq_len):
            dt = current_dt - timedelta(hours=self.time_interval * i)
            t_str = dt.strftime("%Y%m%d%H")
            era_keep = self.load_files(t_str)
            t_feat = get_cyclic_time(dt)
            if era_keep is None:
                if len(frames[0]) > 0:
                    frames[0].append(frames[0][-1])
                    frames[1].append(t_feat)
                else:
                    return None
            else:
                frames[0].append(era_keep)
                frames[1].append(t_feat)
    ...
    def __getitem__(self, idx: int):
        ...
        inputs = {
            "era_keep": torch.from_numpy(seq_data[0]),
            "time_cyclic": torch.from_numpy(seq_data[1]),
        }
        _, _, H, W = inputs["era_keep"].shape
...
def run_single_seed(seed: int) -> Dict[str, Any]:
    ...
    print("--- MST_DETR_ERA5Only: ERA5-only input and backbone | seed={} ---".format(seed))
    ...
    print("Ablation setting          : GridSat input and the satellite branch are removed.")
